...
(function() {
  var BOX_NONCE_BYTES, CIPHER_DECRYPT_INFO_FIELD, CIPHER_MESSAGE_FIELD, CIPHER_TRANSIENT_PKEY_FIELD, CIPHER_VERSION, CIPHER_VERSION_FIELD, DECRYPT_INFO_MESSAGE_INFO_FIELD, DECRYPT_INFO_SENDER_FIELD, DisturbeApp, EncryptMessage, GeneratePrivateKey, HELLO_CLIENT_TRANSIENT_PKEY_FIELD, HELLO_PADDING_BYTES, HELLO_PADDING_FIELD, HELLO_URL, HELLO_ZEROS_BOX_FIELD, INITIATE_CLIENT_TRANSIENT_PKEY_FIELD, INITIATE_COOKIE_FIELD, INITIATE_URL, INITIATE_VOUCH_FIELD, InputField, KEY_BASE64_BYTES, KeyCabinet, KeyProfile, KeyProfileItem, MESSAGE_INFO_KEY_FIELD, MESSAGE_INFO_NONCE_FIELD, MINIMUM_PASSWORD_ENTROPY_BITS, PublicKeyField, SCRYPT_L, SCRYPT_N, SCRYPT_P, SCRYPT_R, SERVER_DOMAIN_NAME, SERVER_PUBLIC_KEY, SecretKeyField, VOUCH_CLIENT_PKEY_FIELD, VOUCH_DOMAIN_NAME_FIELD, VOUCH_MESSAGE_FIELD, VOUCH_TRANSIENT_KEY_BOX_FIELD, VerifyPassword, a, b64decode, b64encode, br, button, concatBuffers, credentialsToSecretKey, decode_utf8, decryptMessage, div, encode_utf8, encryptMessage, form, h1, h2, h3, h4, h5, h6, hr, i, input, label, li, nacl, option, p, scrypt, select, sendHello, sendInitiate, sendMessage, span, strong, textarea, toHex, ul, validPublicKey, _ref;
  ...
  DisturbeApp = React.createClass({
    ...
    render: function() {
      return div(null, h1({
      ...
      }, 'Sign in with your public key'))) : div({
        className: 'row'
      }, div({
        className: 'col-md-12 large-bottom'
      }, span({
        className: 'text-monospace text-muted'
      }, 'Successfully retrieved your public key profile.'))), div({
        className: 'row'
      }, div({
        className: 'col-md-12'
      }, hr(null))), div({
        ...
        className: 'text-monospace'
      }, 'Compose a message and encrypt it. Only the owners of the public keys you specify will be able to decrypt it.')), div({
        className: 'col-md-12 large-bottom'
      }, EncryptMessage({
        userKeys: this.state.userKeys
      })))) : GeneratePrivateKey({
  ...
  KeyProfile = React.createClass({
    ...
    render: function() {
      return div(null, h3({
        className: 'media-heading text-monospace'
  ...
  EncryptMessage = React.createClass({
    ...
    encryptMessage: function(event) {
      var cipher, error, key, recipientKeys, recipientNode;
      ...
      try {
        cipher = encryptMessage(this.props.userKeys, recipientKeys, this.state.message);
        return this.setState({
          message: cipher
        });
      } catch (_error) {
    ...
    decryptMessage: function(event) {
      var error, message, plaintext, sender;
      event.preventDefault();
      try {
        plaintext = decryptMessage(this.props.userKeys, this.state.message);
        plaintext.message = decode_utf8(plaintext.message);
        sender = plaintext.sender, message = plaintext.message;
        return this.setState({
          message: JSON.stringify(plaintext)
        });
      } catch (_error) {
        error = _error;
        return console.log(error);
    ...
    render: function() {
      ...
      return form({
        ...
        className: 'text-monospace text-danger'
      }, error))) : void 0, div({
        className: 'form-group'
      }, div({
        className: 'col-xs-12',
        ...
        className: 'text-monospace control-label'
      }, 'Recipients'), input({
      ...
      }, 'Decrypt'), button(encryptButtonProps, i({
